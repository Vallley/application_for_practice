...
class TestModels(TestCase):
    ...
    def test_productions_model(self):
    ...
    def test_firm_model(self):
        ...
        self.assertEquals(
            firm._meta.get_field("main_inf").verbose_name, "Основная информация"
        )
        ...
        self.assertEquals(
            firm._meta.get_field("productions").verbose_name, "Вид продукции"
        )
    ...
    def test_person_model(self):
        ...
        self.assertEquals(
            person._meta.get_field("main_inf").verbose_name, "Основная информация"
        )
        ...
        self.assertEquals(
            person._meta.get_field("nationality").verbose_name, "Национальность"
        )
        self.assertEquals(person._meta.get_field("nationality").max_length, 100)
        self.assertEquals(
            person._meta.get_field("family_status").verbose_name, "Семейный статус"
        )
        self.assertEquals(person._meta.get_field("family_status").max_length, 100)
        self.assertEquals(
            person._meta.get_field("profession").verbose_name, "Профессия"
        )
        self.assertEquals(person._meta.get_field("profession").max_length, 100)
        self.assertEquals(
            person._meta.get_field("education").verbose_name, "Образование"
        )
        self.assertEquals(
            person._meta.get_field("experience").verbose_name, "Опыт работы"
        )
